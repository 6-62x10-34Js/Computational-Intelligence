...
def main():
    ...
    plot_error_ranges_of_degree_reg(reg_error_train_D, reg_error_test_D, reg_degree)
...
def plot_error_ranges_of_degree(error_train_D, error_test_D, reg_degree):
    ...
    ax.set_ylabel('Error')
    ax.set_title('Error of Linear Regression')
    ax.legend()
    plt.savefig('plots/error_of_linear_regression.png')
    plt.show()


    difference = error_train_D - error_test_D
    ...
    ax.set_title('Difference of Training and Test Error')
    ax.legend()
    plt.savefig('plots/difference_of_training_and_test_error.png')
    plt.show()




def plot_error_ranges_of_degree_reg(reg_error_train_D, reg_error_test_D, reg_degree):
    """ plots the training and test errors against the degree of the polynomial.
    ...
    ax.set_xlabel('Degree of Polynomial')
    ax.set_ylabel('Error')
    ax.set_title('Error of Linear Regression with Regularization')
    ax.legend()
    plt.savefig('plots/error_of_linear_regression_reg.png')
    plt.show()
    ...
    ax.set_title('Difference of Training and Test Error')
    ax.legend()
    plt.savefig('plots/difference_of_training_and_test_error_reg.png')
    plt.show()
...
def plot_range_of_degree(data_train, data_test, degree_range, w_train_reg_D):
    ...
    for i in range(len(degree_range)):
        ...
        ax.plot(x, y, label='Fit with degree = {}'.format(degree_range[i]))
    ...
    ax.legend()
    plt.savefig(f'plots/lin_range_of_degree_{degree_range}_trainingdata.png')
    plt.show()
    ...
    ax.legend()
    plt.savefig(f'plots/lin_range_of_degree_{degree_range}_testdata.png')
...
def opt_weight_reg(data, Phi, lambda_reg):
    ...
    w_star_reg = np.dot(np.dot(np.linalg.inv(np.dot(Phi.T, Phi) + lambda_reg * np.identity(len(Phi.T))), Phi.T), data[:, 1])
